...
if limited_range
    center_theta = pi/6;
    center_phi = pi;
    anglerange_deg = 6; %Sets range of angles used if limited_range is true
...
domainsize = 10; %in ?m, x-y dimensions of box to integrate
...
for k = 1 : length(theFiles)
    ...
    save('saved_field_matrix.mat', 'E', 'H', 'obj_array', 'src_array')
    ...
    %% Calculate init
    init = retop(@FieldInterpolation_onbox_v2,k0,n_strates,z_strates,Centre,L,N_gauss,struct('option_cal_champ',option_cal_champ,'option_i',option_i));
    ...
    save(savefilename, 'angles_h', 'init', 'Fh', 'uh', 'vh', 'u', 'v', 'w')
