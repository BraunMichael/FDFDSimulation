function varargout=retop_inenglish(varargin)
% development in plane waves in a homogeneous medium of index n
%%%%%%%%%%%%%%%%%%%%%%%
%% 2 D%
%%%%%%%%%%%%%%%%%%%%%%%
...
%
%
% n: homogeneous medium index k0 = 2 * pi / ld
% u, v: incidences where we want to calculate the development in plane waves
% E = sum_on_u_v of (tf (E) * exp (i (u * x + v * y)))
% e (z, x, y, 1: 6) computed field by retchmp (6 components)
% wx, wy, wz: weight for Fourier integration
% one of these vectors is empty: the development in plane waves is made in the perpendicular direction
% parm = struct ('apod', 0, 'delt', 0, 'uvmesh', 0); (by default)
%   apod: for Fourier integration
%   delt: for singularity of normal incidence
% if uvmesh = 1 u and v are vectors of the same dimension giving the couples of values ??of u and v
% otherwise u and v are 2 vectors. The calculation is done on ndgrid (u, v)
% in the following tables u, v are then 'welded' and these tables lose a dimension
%
% if wz = [] Ep (u, v, 1: 6, z), Em (u, v, 1: 6, z), are the Fourier components of e (z, x, y, 1: 6) with respect to x, y functions of z (x-> u y-> v)
% if wx = [] Ep (u, v, 1: 6, x), Em (u, v, 1: 6, x), are the Fourier components of e (z, x, y, 1: 6) with respect to y, z functions of x (y-> u z-> v)
% if wy = [] Ep (u, v, 1: 6, y), Em (u, v, 1: 6, y), are the Fourier components of e (z, x, y, 1: 6) with respect to z, x functions of y (z-> u x-> v)
% these components are defined in the same reference that e (Ep (u, v, 1, z) is Ex etc ...)



%*******************************************************************************
% formatting of plane waves in 2D (if angles is output)
%
% if length (u) = nu, length (v) = nv, length (z) = nz:
% size (Ep) = size (Em) = [nu, nv, 6, nz]
% teta: [nu nv] angle of oz with kp (0 a pi / 2)
% delta: [nu nv] angle of oy with vp oriente by oz (-pi a pi)
% psip: [nu nv nz] angle (kp-oriented) between up and the main polarization direction of Ep
% kp: [nu nv 3] UNITARY wave vector directs upwards
% vp: [nu nv 3] unit perpendicular to the plane of incidence (the zero 0re, kp, vp being direct)
% up: [nu nv 3] up = vp / \ kp (the triedre up vp kp is direct)
% EEp: [nu nv 2 nz] components of Ep on up and vp
% HHp: [nu nv 2 nz] components of Hp on up and vp
% EEEp: [nu nv 2 nz] amplitude of Ep in the index deduced from up vp by angle rotation psip
% HHHp: [nu nv 2 nz] amplitude of Ep in the index deduced from up vp by psip angle rotation
% angles. Fp: [nu nv nz] diagram of upward diffraction in energy (* 1 / r ^ 2 ...)
% psim: [nu nv nz]: angle (oriente per km) between um and the main polarization direction of Em
% km: [nu nv 3] UNITARY wave vector directs down
% vm: [nu nv 3] = unit vm perpendicular to the plane of incidence (the zero 0re, km, vm is direct)
% um: [nu nv 3] = vm / \ km (the provenre um vm km is direct)
% EEm: [nu nv 2 nz] components of Em on um and vm
% HHm: [nu nv 2 nz] components of Hm on um and vm
% EEEm: [nu nv 2 nz] amplitude of Em in the reference derived from um vm by psim angle rotation
% HHHm: [nu nv 2 nz] amplitude of Hm in the index deduced from um vm by psim angle rotation
% Fm: [nu nv nz] energy down diffraction diagram (* 1 / r ^ 2 ...)
% for normal incidence (degenerate case) vp = vm = [- sin (delt), cos (delt)]
% (the delt angle can be entered in parameter and is 0 by default)
...
%
% n: homogeneous medium index k0 = 2 * pi / ld pol: 0 E // 2 H //
% u: incidences where one wants to calculate the development in plane waves (k0 * n * sin (teta))
% E = sum_on_u of (tf (E) * exp (i (u * x))
% e (y, x, :) field computed by retchamp (3 or 4 components)
% wx, wy: weight for Fourier integration
% one of these vectors is empty: the development in plane waves is made in the perpendicular direction
% parm = struct ('apod', 0); (by default)
% apod: for the Fourier integration
%
% if wx = [] Ep (u, 1: 3, x), Em (u, 1: 3, x), are the Fourier components of e (y, x, 1: 3) with respect to y functions of x
% if wy = [] Ep (u, 1: 3, y), Em (u, 1: 3, y), are the Fourier components of e (y, x, 1: 3) with respect to x functions of there
%
% ******************************
% formatting of plane waves in 1D (if angles is output)
% if length (u) = naked, length (y) = ny:
% size (Ep) = size (Em) = [nu, 3, nz]
% teta: [nu 1] angle of kp with Oy oriented by Oz (-pi / 2 a pi / 2)
% kp: [nu 2] UNITARY wave vector goes up
% up: [nu 2] perpendicular to kp (Oz up kp direct)
% EEp: [nu ny] component of Ep on Oz
% HHp: [nu ny] component of Hp on up
% angles. Fp: [nu ny] upward diffraction diagram in energy (* 1 / r ...)
% km: [nu 2] UNITARY wave vector directs down
% um: [nu 2] perpendicular to km (Oz um km direct)
% EEm: [nu ny] component of Em on Oz
% HHm: [nu ny] component of Hm on um
% Fm: [nu ny] energy down diffraction diagram (* 1 / r ...)
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Haitao correction in 1D %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Plasmons must have been calculated previously by retpl (or otherwise)
% e must be replaced by: {e, Plg, Pld} where: Pl = {pl, x0, [nh, nb], y_dioptre}
% pl: amplitude of the plasmon as given by retpl: thus normalized by the flux of the vector of poynting in x = 0
% pl is a scalar (average or one-point value)
% x0: point from which the plasmon is added
% nh: at the top
% nb: below
% y_dioptre: metal diopter
% In this case the apodization only acts on the field minus the plasmon
%
%
%
% 		%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 		%      METHOD OF INTEGRAL OR BOX        %
% 		%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%We integrate Lorentz on a box outside of which the medium is laminated
% one of the fields is the calculated one (which must satisfy COS)
% the other is the response of the stratified medium to an incident plane wave
%%%%%%%%%%%%%%%%%
%% 1 D%
%%%%%%%%%%%%%%%%%
% angles = retop (n, y, u, e_x, x_x, y_x, w_x, e_y, x_y, y_y, w_y, pol, k0, sense);
%
% n: indices from top to bottom
% y: diopter dimensions from top to bottom (same origin as in the field calculation)
% u: incidences where one wants to calculate the development in plane waves (k0 * n * sin (teta))
% e_x, x_x, y_x, w_x field calculated by retchamp on 2 sections in y (y_x) w_x weight for the integration in x
% e_y, x_y, y_y, w_y field calculated by retchamp on 2 sections in x (x_y) w_y weights for integration in y
% real (meaning) 1 development above in y (1) imag (direction) 0 strata are perpendicular to oy
% -1 development below in y (end) 1 strata are perpendicular to ox
% y must not be []. For a homogeneous medium with 2 equal elements
% angles structure to fields: 'teta', 'k', 'u', 'EE', 'HH', 'F', 'Flux_poynting'
%
% Modes
%-------
% angles = retop ([n, neff], y, e_x, x_x, y_x, w_x, e_y, x_y, y_y, w_y, pol, k0, sense);
% be careful not to put pol after neff
% angles structure to field fields: amp_p, Fp = abs (amp_p) ^ 2 (right mode)
% amp_m, Fm = abs (amp_m) ^ 2 (left mode)
% Flux_pornting (on the box)
% and also the 0D (uncloned) fields used to generate the modes

%
%       %%%%%%%%%%%%%%
% 		%     2 D    %
% 		%%%%%%%%%%%%%%
% angles = retop (n, z, u, v, e_xy, x_xy, y_xy, z_xy, wx_xy, wy_xy, e_zx, x_zx, y_zx, z_zx, wz_zx, wx_zx, e_yz, x_yz, y_yz, z_yz, wy_yz, wz_yz, k0, sense parm);
% e_xy, x_xy, y_xy, z_xy, wx_xy, wy_xy, ...% field on 2 sections perpendicular to oz with points and weights
% e_zx, x_zx, y_zx, z_zx, wz_zx, wx_zx, ...% field on 2 sections perpendicular to oy with points and weights
% e_yz, x_yz, y_yz, z_yz, wy_yz, wz_yz, ...% field on 2 sections perpendicular to ox with points and weights
% real (meaning) = 1 development above in z (1) imag (direction) = 0 strata are perpendicular to oz
% real (meaning) = - 1 development below in z (end) imag (direction) = 1 the strata are perpendicular to ox
% imag (meaning) = -1 the strata are perpendicular to oy
% z must not be []. For a homogeneous medium with 2 equal elements
% parm structure defaultopt = struct ('uvmesh', 0, 'clone', 0, 'test', 0);
% if uvmesh = 1 u and v are vectors of the same dimension giving the couples of values ??of u and v
% Field structure angles: teta, delta, psi, k, v, u, EE, HH, EEE, HHH, F, 'Flux_poynting'
%
% Cylindrical modes
%-----------------------
%
% angles = retop ([n, neff, pol], z, LL, teta, e_xy, x_xy, y_xy, z_xy, wx_xy, wy_xy, e_zx, x_zx, y_zx, z_zx, wz_zx, wx_zx, e_yz, x_yz, y_yz, z_yz, wy_yz, wz_yz, k0, direction);
% imag (sense) as for plane waves (only the imaginary part is used to tell the direction of strata)
% if LL = [] direct calculation for teta
% if LL is non-empty if LL = L1 LLL = L1, if LL = [L1, L2] LLL = L1: L2, if LL = [L1, L2, L3], LLL = L1: L2: L3, else LL = retelimine from LL
% if LL is a column vector LLL = LL.
% be careful with, if LL = -1: 1 we get LLL = -1: 0: 1 = [] so it's better to do LL = (- 1: 1). '
% calculation of the Fourier coefficients LLL, then Fourier synthesis in teta
% angles structure to field fields f, amp, F f Fourier coefficients, amp amplitude (teta) F intensity (teta), LLL
% and also the 0D (uncloned) fields used to generate the modes.
% if [teta, wteta] = retgauss (0.2 * pi, ...), the energy on the mode is sum (angles.F. * wteta (:)) or 2 * pi * sum (abs (f ). ^ 2)
%
% Choon Modes
%-----------------------
...
%     |-1-i  |   Oz       |  x     |  y       |
%     -----------------------------------------
% e_mode, x_mode, z_mode, neff calculated by retmode neff = gama / k0
% parm optional field structure (if parm.test = 1, print fields on the box)
%
%
%
%       %%%%%%%%%%%%%%%%%%%%%%%%%%%
% 		%    cylindres Popov      %
% 		%%%%%%%%%%%%%%%%%%%%%%%%%%%
% angles = retop (n, z, u, v, e_x, x_x, z_x, wx_x, e_z, x_z, z_z, wz_z, k0, sense, L, sym, parm);
% e_x, x_x, z_x, wx_x field on 2 sections perpendicular to oz x_x ranging from 0 to r with points and weights
% e_z, x_z, z_z, wz_z field for x_z = r
% VERY IMPORTANT:
% The computation of the fields must be IMPERATIVELY carried out with the option sym = 0 even if one computes the development with sym = 1 or -1
% (recalculate init or do init {end} .sym = 0 before retchamp)
%
% meaning 1 development above in z (1)
% -1 development below in z (end)
% L sym popov settings
%
% parm, u, v, angles: as in 2D
%
% Modes cylindriques
%-----------------------
% angles = retop ([n, neff, pol], z, L, teta, e_x, x_x, z_x, wx_x, e_z, x_z, z_z, wz_z, k0, sym);
% angles structure to field fields f, amp, F f Fourier coefficient on L, amp amplitude (teta) taking into account the sym parameter
% F intensity (teta) taking into account the sym parameter and also the 0D (uncloned) fields used to generate the modes
%--------------------------------------------------------------------------
%     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     %  AUTOMATION OF THE BOX METHOD  %
%     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% INITIALISATION
%
% init = retop (cal_field, k0, n_strates, z_strates, Center, L, N_gauss, parm); in 2D
% init = retop (cal_field, pol, k0, n_strates, z_strates, Center, L, N_gauss, parm); in 1D
% init = retop (cal_field, k0, n_strates, z_strates, L_popov, z_Center, HR, N_gauss, parm); Popov cylinders
%
%
% parm = struct ('orientation', 0, 'disc', {{[], [], []}},'calfield_option ', 2,'SI_option', 0); by default
% if option_cal_field = 0:
% cal_field must be a function that calculates the uncloned field in retchamp format
% e = calchamp (x (:), y (:), z (:)) size e = [length (z (:)), length (x (:)), length (y (:)), 6] in 2D
% e = calchamp (x (:), y (:)) size e = [length (y (:)), length (x (:)), 3] in 1D
% if field_scale = 1 or 2:
% cal_field should be a function that calculates the uncloned field in the following format
% e = calchamp (x (:), y (:), z (:)) size e = [length (x (:)) = length (y (:)) = length (z (:)), 6] in 2D
% e = calchamp (x (:), y (:)) size e = [length (x (:)) = length (y (:)), 3] in 1D
% if field_scale = 2 the field_call is only called once
% option_SI <0 exp (-iwt) option_SI> 0 exp (iwt)
% Scale = abs (option_SI) unit of length in meters (ld x y z k0 u v)
% if option_SI ~ = 0 E and H are normalized with the factor Z0
% E * sqrt (Z0) H / sqrt (Z0)
% option_SI = 1 comsol
% option_SI = -1.e6 conventions reticolo
% option_SI = 0 reticolo conventions
%
% k0 = 2 * pi / ld;
% n_strates, z_strates, length (n_strates) = 1+ length (z_strates)> 2 from top to bottom
% z_strates can not be []
% THE ORIGIN of the DOP is z_strates (1) if meaning = 1 (top)
% z_strates (end) if meaning = -1 (bottom)
% Center: Center of the box (length 3 in 2D, 2 in 1D)
% L widths of the box
% N_gauss for gauss points size chart [3,2]
% by default: N_gauss = [[10,10]; [10,10]; [10,10]])
% first column: degree, second repetition
% The second column is optional
%
% orientation, optional discs
% in 2D:
% orientation: 0 (default) strata are perpendicular to oz
% 1 strata are perpendicular to ox
% -1 strata are perpendicular to oy
% in 1D:
% orientation: 0 (default) strata are perpendicular to oy
% 1 strata are perpendicular to ox
% disc cell array of discontinuities for gauss
%

% CALCULATION OF DOP
% in 2D:
% angles = retop (init, u, v, direction, parm);
% u v: K //
% meaning = 1 above, -1 below
% parm = struct ('uvmesh', 1, 'test', 0); optional default test = 1: box plot (recommended)
% Warning the uvmesh option is the opposite of the classic version: we parametrise more often in teta and phi than in u and v
% in 1D: angles = retop (init, u, direction, parm), parm = struct ('test', 0); by default
%
% Popov: angles = retop (init, u, v, direction, parm);
% parm = struct ('uvmesh', 1, 'sym', 0, 'test', 0); by default
% For modes: in 2D
% angles = retop (init, neff, pol, L, teta, parm); pol = 0 or 2 mode polarization
% L = -5: 5 ... teta is mandatory and can be []
% in 1D: angles = retop (init, neff, parm); parm = struct ('test', 0) by default. test = 1 trace of the box
% popov: angles = retop (init, neff, pol, parm);
% parm = struct ('sym', 0, 'test', 0, 'teta', []); by default
% if [teta, wteta] = retgauss (0.2 * pi, ...), the energy on the mode is sum (angles.F. * wteta (:)) or 2 * pi * sum (abs (f ). ^ 2)%--------------------------------------------------------------------------
%
% % EXEMPLE 2 D
...
% pols=rand(1,6)+i*rand(1,6);e=retpoint(k0*n^2,k0,pols,[0,0,0],x,y,z);% point source
...
%  energie_vers_le_haut=w.'*angles.Fp(:,2) %Energy up
%  energie_vers_le_bas=w.'*angles.Fm(:,1)   %Energy down
%
% %  EXEMPLE 1 D
% %Fourier en x
...
angles=retop_inenglish(n_strates,z_strates,u(:),v(:),champ{:},k0,sens+1i*orientation,struct('option_parfor',option_parfor,'uvmesh',uvmesh,'test',test,'Scale',Scale));
...
angles=retop_inenglish(n_strates,z_strates,LL(:),teta,champ{:},k0,1i*orientation,parm);
if option_SI~=0;% mise en forme et conversion en unite SI
...
angles=retop_inenglish(n_strates,z_strates,u(:),champ{:},pol,k0,sens+1i*orientation,parm);
if option_SI~=0;% mise en forme et conversion en unite SI
    angles.EH=angles.e;
...
if option_SI~=0;Scale=1.e-6/abs(option_SI);else;Scale=1;end;if ~isempty(parm);parm.Scale=Scale;end;
angles=retop_inenglish(n_strates,z_strates,champ{:},pol,k0,1i*orientation,parm);
if option_SI~=0;% mise en forme et conversion en unite SI
...
angles=retop_inenglish(n_strates,z_strates,u(:),v(:),champ{:},sens,L,sym,struct('uvmesh',uvmesh,'test',test));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
[n_strates,z_strates,L,champ]=deal(init{:});
n_strates=[n_strates,neff,pol];
angles=retop_inenglish(n_strates,z_strates,L,teta,champ{:},sym,struct('test',test));
...
[gama,kmax,vm,er,emode,~,ymode]=retmode(pol,n,-diff(y),k0*neff,[],[hrabh,hrabb,0],[],k0);
