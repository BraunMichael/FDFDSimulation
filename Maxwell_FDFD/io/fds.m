...
function [E, H, err] = fds(omega, d_prim, d_dual, s_prim, s_dual, ...
    ...
    if numel(mu) ~= 3
        error('All 3D vector fields (MU) must have three cell elements.');
    end
    if numel(epsilon) ~= 3
        error('All 3D vector fields (EPSILON) must have three cell elements.');
    end
    if numel(E) ~= 3
        error('All 3D vector fields (E) must have three cell elements.');
    end
    if numel(J) ~= 3
        error('All 3D vector fields (J) must have three cell elements.');
    end
