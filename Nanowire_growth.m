...
appendstring = 'siliconNW'; %_background added with createBackgroundFile
...
NWspacing_x = [400]; %[350 400 500 600 700 800]; %This is delta=0 spacing, regardless of number of wires in supercell
        ...
        for j=1:max(size(D))
            ...
            for m=1:max(size(Laser_angle))
                for n=1:max(size(Laser_pol))
                    ...
                    pythonRestartFile(typestring, endstring);
